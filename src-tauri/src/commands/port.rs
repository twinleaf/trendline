...
#[tauri::command]
pub fn connect_to_port(port_url: String, registry: State<'_, Arc<ProxyRegister>>) -> Result<(), String> {
...
#[tauri::command]
pub fn refresh_port(port_url: String, registry: State<'_, Arc<ProxyRegister>>) -> Result<(), String> {
    let pm = registry.get(&port_url).ok_or_else(|| format!("Port '{}' not found", port_url))?;
    pm.rescan();
    Ok(())
}
...
pub fn confirm_selection(
    ...
    let port_manager = registry.get(&port_url).ok_or_else(|| format!("Could not find PortManager for URL: {}", port_url))?;
    ...
    capture.inner.command_tx.send(command).map_err(|e| e.to_string())?;
