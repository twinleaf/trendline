...
/// Spawn a background thread that keeps `ProxyRegister` in sync with whatever
/// Twinleaf-compatible serial devices are visible on the host.
        ...
        .spawn(move || {
            loop {
                let urls: HashSet<String> =
                    get_valid_twinleaf_serial_urls().into_iter().collect();
